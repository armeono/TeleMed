...
export default function ScheduleModal({
  ...
  const handleSchedule = async () => {
    ...
    if (!selectedDate) {
      toast({
        title: "Error",
        description: "Please select a valid date for the appointment.",
      });
      return;
    }
    ...
    if (!selectedTime) {
      toast({
        title: "Error",
        description: "Please select a valid time slot for the appointment.",
      });
      return;
    }
    ...
    const response = await action_scheduleAppointment({
      ...
      time: selectedTime, 
      reason: symptoms,
      type: appointmentType as "IN_PERSON" | "ONLINE", 
      files
    });
...
const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  if (e.target.files) {
    setFiles(Array.from(e.target.files)); // Convert FileList to an array and set the state
  }
};
  ...
  return (
    <div>
      ...
      <p className="text-gray-700 ">
        ...
        {step === 3 && "Select an available time slot."}
        {step === 4 && "Choose the type of appointment."}
        {step === 5 && "Describe your symptoms and upload any relevant files."}
      </p>
...
{/* Step 5: Symptoms and File Upload */}
{step === 5 && (
  <div className="space-y-4">
    <div>
      ...
      <Textarea
        id="symptoms"
        placeholder="Please describe your reason for visit, symptoms, and any relevant medical history..."
        value={symptoms}
    ...
    <div>
      <Label htmlFor="file-upload">Upload relevant files (optional)</Label>
      <div className="mt-2">
        <input
          id="file-upload"
          name="file-upload"
          type="file"
          multiple
          className="hidden" // Ensure the input is visually hidden
      ...
      {files.length > 0 && (
        ...
        <div className="flex justify-between pt-4">
          ...
          <Button
            onClick={step === 5 ? handleSchedule : handleNext}
            disabled={
              (step === 1 && !selectedDoctor) ||
              (step === 2 && !selectedDate) ||
              (step === 3 && !selectedTime) ||
          ...
          >
            {step === 5 ? "Schedule Appointment" : "Next"}
          </Button>
        </div>
      </div>
    </div>
